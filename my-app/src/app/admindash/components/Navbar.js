...
const Navbar = () => {
  ...
  const titleClass = isClient
    ? 'text-xl ml-14 font-extrabold bg-gradient-to-r from-blue-600 to-indigo-600 text-transparent bg-clip-text'
  ...
  return (
    <header className={headerClass}>
      ...
      {isClient ? (
        ...
        <div className={titleClass}>Welcome, Admin!</div>
      )}
      
      <Link
        href="/admindash/profile"
        className="flex items-center justify-center p-2 rounded-full w-10 h-10 bg-gradient-to-r from-blue-500 to-indigo-600 text-white hover:shadow-md transform hover:scale-105 transition-all duration-300"
      >
        <UserCog size={18} />
